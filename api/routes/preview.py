...
async def get_latest_previews(
    ...
    ACCUMULATED, atau SILVER untuk baris pra-D14): raster per-band yang bisa
    di-thumbnail langsung.
    Response berisi metadata + URL thumbnail.
    """
    with db.session() as sess:
        # Ambil produk rank 2 terbaru
        products = sess.scalars(
            select(DataProduct)
            .where(
                and_(
                    DataProduct.product_tier.in_(tn.tiers_at_rank(2)),
